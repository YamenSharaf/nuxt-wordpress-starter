...
module.exports = function() {
  ...
  this.nuxt.hook('generate:before', async generator => {
    const routes = async () => {
      ...
      const promises = generator.nuxt.options.wordpress.postTypes.map(
        async postType => {
        ...
        await Promise.all(promises).then(data => {
          ...
          flatData.map(result => {
            ...
            if ('posts' === postType) {
              ...
              fs.writeFile(
                generator.nuxt.options.generate.apiCacheDir +
                  'page-' +
                  page +
                  '.json',
                JSON.stringify(data),
